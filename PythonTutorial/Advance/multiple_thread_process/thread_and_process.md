...
这就是基本的问题模型，也是消费者-生产者模型的一个例子。让我们看看一个使用Lock的解决方案。这不是一个最佳解决方案
...
起初，你可能会觉得很奇怪，生产者在消费者运行之前就已经得到了两条消息。如果你回头看看生产者和.set_message()，你会注意到它唯一会等待锁的地方是当它试图将消息放入管道时。这是在生产者得到消息并记录下它拥有消息后进行的。
...
Python 的标准库有一个队列模块，它又有一个 Queue 类。让我们改变管道来使用一个队列，而不仅仅是一个由锁保护的变量。你还将使用一种不同的方式来停止工作线程，即使用Python线程中的一个不同的基元--事件。
